...
const UserList = ({ setTeam, selectedTeam }) => {
  const { teams } = useSelector((state) => state.teams);
  const [selectedUsers, setSelectedUsers] = useState(selectedTeam);
  ...
  useEffect(() => {
    if (selectedTeam) {
      ...
      selectedTeam.map((team) => {
        teams.map((item) => {
          if (item._id == team._id) temp.push(item);
        });
      });

      setSelectedUsers(temp);
      setTeam(selectedTeam?.map((u) => u._id));
    }
  }, [teams]);
