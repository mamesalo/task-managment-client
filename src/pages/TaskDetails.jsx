...
const Activities = ({ activity, id }) => {
  ...
  const handleSubmit = async () => {
    if (!selected || !text) {
      enqueueSnackbar("Enter All Field", {
        variant: "error",
      });
      return;
    }
    ...
    axios
      .post(
        `${import.meta.env.VITE_API_URL}/api/task/activity/${id}`,
        { type: selected.toLowerCase(), activity: text },
        {
          headers: {
      ...
      .then((response) => {
        dispatch(
          setTasks(
            tasks.map((task) =>
              task._id == id
                ? { ...task, activities: response.data.task.activities }
                : task
            )
          )
        );
        ...
        setText("");
        enqueueSnackbar("Activity posted Created Succesfully", {
          variant: "success",
        });
      })
      .catch((error) => {
    ...
    <div className="w-full flex gap-10 2xl:gap-20 min-h-screen px-10 py-8 bg-white shadow rounded-md justify-between overflow-y-auto">
      ...
      {task?.stage == "in progress" ? (
        ...
        <p className="text-xl font-semibold">
          Start/Restart the Task to add comments
        </p>
      )}
    </div>
  );
